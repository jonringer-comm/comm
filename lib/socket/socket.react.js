...
class Socket extends React.PureComponent<Props, State> {
  ...
  initialPlatformDetailsSent: boolean =
  ...
  sendMessageWithoutID: (
    message: ClientSocketMessageWithoutID,
  ) => number = message => {
  ...
  async sendInitialMessage() {
    ...
    const initialMessage = {
      ...
      id: messageID,
      payload: {
        clientResponses: clientResponses.map(response =>
          convertClientClientResponse(response, 'client_to_server'),
        ),
        sessionState: convertSessionState(sessionState, 'client_to_server'),
