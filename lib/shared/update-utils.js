...
import { convertRawEntryInfos, convertRawEntryInfo } from './entry-utils';
import { convertRawMessageInfos } from './message-utils';
import { convertRawThreadInfo, convertThreadID } from './thread-utils';
...
function convertClientCreateUpdatesResponse(
  clientCreateUpdatesResponse: ClientCreateUpdatesResponse,
  conversionType: ConversionType,
): ClientCreateUpdatesResponse {
  return {
    ...clientCreateUpdatesResponse,
    viewerUpdates: clientCreateUpdatesResponse.viewerUpdates.map(update =>
      convertClientUpdateInfo(update, conversionType),
    ),
  };
}

function convertClientUpdateInfo(
  clientUpdateInfo: ClientUpdateInfo,
  conversionType: ConversionType,
): ClientUpdateInfo {
  const result = { ...clientUpdateInfo };
  if (result.threadInfo) {
    result.threadInfo = convertRawThreadInfo(result.threadInfo, conversionType);
  }
  if (result.threadID) {
    result.threadID = convertThreadID(result.threadID, conversionType);
  }
  if (result.rawEntryInfos) {
    result.rawEntryInfos = convertRawEntryInfos(
      result.rawEntryInfos,
      conversionType,
    );
  }
  if (result.rawMessageInfos) {
    result.rawMessageInfos = convertRawMessageInfos(
      result.rawMessageInfos,
      conversionType,
    );
  }
  if (result.entryInfo) {
    result.entryInfo = convertRawEntryInfo(result.entryInfo, conversionType);
  }
...
export {
  ...
  convertClientCreateUpdatesResponse,
  convertClientUpdateInfo,
};
