...
import { convertRawMessageInfos } from './message-utils';
import { convertThreadID } from './thread-utils';
import { convertClientCreateUpdatesResponse } from './update-utils';
...
function convertCalendarQuery(
  calendarQuery: CalendarQuery,
  conversionType: ConversionType,
): CalendarQuery {
  return {
    ...calendarQuery,
    filters: calendarQuery.filters.map(calendarFilter =>
      calendarFilter.threadIDs
        ? {
            ...calendarFilter,
            threadIDs: calendarFilter.threadIDs.map(threadID =>
...
function convertRawEntryInfos(
  rawEntryInfos: $ReadOnlyArray<RawEntryInfo>,
  conversionType: ConversionType,
): $ReadOnlyArray<RawEntryInfo> {
  return rawEntryInfos.map(rawEntryInfo =>
    convertRawEntryInfo(rawEntryInfo, conversionType),
  );
}

function convertRawEntryInfo(
  rawEntryInfo: RawEntryInfo,
  conversionType: ConversionType,
): RawEntryInfo {
  return {
    ...rawEntryInfo,
    threadID: convertThreadID(rawEntryInfo.threadID, conversionType),
  };
}

function convertCreateEntryPayload(
  createEntryPayload: CreateEntryPayload,
  conversionType: ConversionType,
): CreateEntryPayload {
  return {
    ...createEntryPayload,
    threadID: convertThreadID(createEntryPayload.threadID, conversionType),
    newMessageInfos: convertRawMessageInfos(
      createEntryPayload.newMessageInfos,
      conversionType,
    ),
    updatesResult: convertClientCreateUpdatesResponse(
      createEntryPayload.updatesResult,
      conversionType,
    ),
  };
}

function convertHistoryRevisionInfos(
  historyRevisionInfos: $ReadOnlyArray<HistoryRevisionInfo>,
  conversionType: ConversionType,
): $ReadOnlyArray<HistoryRevisionInfo> {
  return historyRevisionInfos.map(
    (historyRevisionInfo: HistoryRevisionInfo) => ({
      ...historyRevisionInfo,
      threadID: convertThreadID(historyRevisionInfo.threadID, conversionType),
    }),
  );
}
