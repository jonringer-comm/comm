...
function convertRawMessageInfo<T: RawMessageInfo>(
  message: T,
  ...
  if (result.sourceMessage) {
    result.sourceMessage = convertRawMessageInfo(
      result.sourceMessage,
      conversionType,
    );
  }

  if (result.initialThreadState && result.initialThreadState.parentThreadID) {
    // $FlowFixMe
    result.initialThreadState = {
      // $FlowFixMe
      ...result.initialThreadState,
      parentThreadID: convertThreadID(
        result.initialThreadState.parentThreadID,
        conversionType,
      ),
    };
  }
  ...
  if (result.childThreadID) {
    result.childThreadID = convertThreadID(
...
function convertRawMessageInfos(
  messages: $ReadOnlyArray<RawMessageInfo>,
  conversionType: ConversionType,
): $ReadOnlyArray<RawMessageInfo> {
  return messages.map(message =>
    convertRawMessageInfo(message, conversionType),
  );
}

function convertMessageTruncationStatuses(
  messageTruncationStatuses: MessageTruncationStatuses,
  conversionType: ConversionType,
): MessageTruncationStatuses {
  return _mapKeys(threadID => convertThreadID(threadID, conversionType))(
    messageTruncationStatuses,
  );
}

function convertNewMessagesPayload(
  newMessagesPayload: NewMessagesPayload,
  conversionType: ConversionType,
): NewMessagesPayload {
  return {
    messagesResult: {
      ...newMessagesPayload.messagesResult,
      rawMessageInfos: convertRawMessageInfos(
        newMessagesPayload.messagesResult.rawMessageInfos,
        conversionType,
      ),
    },
  };
