...
import { getRelativeMemberInfos } from '../selectors/user-selectors';
...
import { convertCalendarQuery } from './entry-utils';
...
function threadOrParentThreadHasAdminRole(
  ...
  return (
    threadInfo.members.filter(member => memberHasAdminPowers(member)).length > 0
  );
}
...
// Temporary placeholder for thread ID schema conversion between client
// and server. It will prepend thread ID with real keyserver ID in the future
function convertThreadID<T: ?string>(
  threadID: T,
  conversionType: ConversionType,
): T | string {
  if (getConfig().platformDetails?.platform === 'web' || !threadID) {
    return threadID;
  }
...
function convertRawThreadInfo(
  rawThreadInfo: RawThreadInfo,
  conversionType: ConversionType,
): RawThreadInfo {
  return {
    ...rawThreadInfo,
    id: convertThreadID(rawThreadInfo.id, conversionType),
    parentThreadID: convertThreadID(
      rawThreadInfo.parentThreadID,
      conversionType,
    ),
    containingThreadID: convertThreadID(
      rawThreadInfo.containingThreadID,
      conversionType,
    ),
    members: convertMemberInfos(rawThreadInfo.members, conversionType),
    currentUser: convertThreadCurrentUserInfo(
...
function convertRawThreadInfos(
  rawThreadInfos: {
    +[id: string]: RawThreadInfo,
  },
  conversionType: ConversionType,
): { +[id: string]: RawThreadInfo } {
  const updatedRawThreadInfos: { [id: string]: RawThreadInfo } = {};

  for (const threadID in rawThreadInfos) {
    updatedRawThreadInfos[
      convertThreadID(threadID, conversionType)
    ] = convertRawThreadInfo(rawThreadInfos[threadID], conversionType);
  }
  return updatedRawThreadInfos;
}

function convertMemberInfos(
  memberInfos: $ReadOnlyArray<MemberInfo>,
  conversionType: ConversionType,
): $ReadOnlyArray<MemberInfo> {
  return memberInfos.map((memberInfo: MemberInfo) => ({
    ...memberInfo,
    permissions: convertThreadPermissionsInfo(
...
function convertThreadCurrentUserInfo(
  currentUserInfo: ThreadCurrentUserInfo,
  conversionType: ConversionType,
): ThreadCurrentUserInfo {
  return {
    ...currentUserInfo,
...
function convertThreadPermissionsInfo(
  threadPermissionInfo: ThreadPermissionsInfo,
  conversionType: ConversionType,
): ThreadPermissionsInfo {
  const resultPermissions = {};
  for (const threadPermission: ThreadPermission of Object.keys(
    threadPermissionInfo,
  )) {
    const originalThreadPermission = threadPermissionInfo[threadPermission];
    if (originalThreadPermission.source) {
      resultPermissions[threadPermission] = {
        ...originalThreadPermission,
        source: convertThreadID(
          originalThreadPermission.source,
          conversionType,
        ),
      };
    } else {
      resultPermissions[threadPermission] = originalThreadPermission;
    }
  }
...
function convertUpdateThreadRequest(
  updateThreadRequest: UpdateThreadRequest,
  conversionType: ConversionType,
): UpdateThreadRequest {
  const updatedChanges = updateThreadRequest.changes.parentThreadID
    ? {
        ...updateThreadRequest.changes,
        parentThreadID: convertThreadID(
          updateThreadRequest.changes.parentThreadID,
          conversionType,
        ),
      }
    : updateThreadRequest.changes;
  return {
    threadID: convertThreadID(updateThreadRequest.threadID, conversionType),
    changes: updatedChanges,
  };
}

function convertClientNewThreadRequest(
  ...
  const result = {
    ...
    calendarQuery: convertCalendarQuery(
      clientNewThreadRequest.calendarQuery,
      conversionType,
    ),
  };
  if (clientNewThreadRequest.id) {
    result.id = convertThreadID(clientNewThreadRequest.id, conversionType);
  }
  if (clientNewThreadRequest.parentThreadID) {
    result.parentThreadID = convertThreadID(
      clientNewThreadRequest.parentThreadID,
      conversionType,
    );
  }
  return result;
...
export {
  ...
  convertMemberInfos,
  convertThreadCurrentUserInfo,
  convertThreadPermissionsInfo,
